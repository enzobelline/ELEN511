...
*/
  ...
  while(!done){
     ...
     ********************************/
      ...
      if((rs_array[i].op ==ADD)&& eq0){//if fp rs and RS[r].Qj ==0 and RS[r].Qk ==0 
        if(is_add_available){
      ...
      if((rs_array[i].op ==SUB)&& eq0){//if fp rs and RS[r].Qj ==0 and RS[r].Qk ==0 
        if(is_add_available){
          is_add_available=false;
          rs_array[i].in_exec=true;
        }
    ...
    if(num_issued_inst < NUM_OF_INST) {
      ...
      if(cand_rs_id!=-1) {
        ...
        if(inst[num_issued_inst].op==ADD || inst[num_issued_inst].op==SUB || || inst[num_issued_inst].op==ADDI|| inst[num_issued_inst].op==BNE ||
